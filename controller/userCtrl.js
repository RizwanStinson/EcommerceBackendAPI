...
const loginAdmin = asyncHandler(async (req, res) => {
  ...
  if (findAdmin.role !== "admin") throw new Error("Not Authorized as Admin");
...
//a user places an order using Cash On Delivery
const createOrder = asyncHandler(async (req, res) => {
  const { COD, couponApplied } = req.body;
  const { _id } = req.user;
  validateMongodbId(_id);
  try {
    if (!COD) throw new Error("Create cashon delivery order failed");
    const user = await User.findById(_id);
